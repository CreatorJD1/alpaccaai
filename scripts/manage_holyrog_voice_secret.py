"""Manage the separate HOLYROG XTTS credential without echoing its value."""
...
def _decode_credential_blob(value: object, *, source: str) -> str:
    ...
    if isinstance(value, bytes):
        # CredWrite stores str blobs as UTF-16-LE. UTF-8 decoding can succeed
        # while silently preserving NULs, so detect the Windows representation.
...
def _derive_voice_secret(compute_secret: str) -> str:
    """Domain-separate the XTTS credential without exposing the parent."""
...
def _write(value: str) -> None:
    ...
    try:
        ...
        raise VoiceSecretError(
            "could not store the HOLYROG XTTS credential"
        ) from exc
...
def _stage(path: Path, value: str) -> None:
    ...
    try:
        ...
        temporary.write_text(
            _validate(value, source="the staged HOLYROG XTTS secret"),
            encoding="utf-8",
        )
        os.replace(temporary, resolved)
    except OSError as exc:
        raise VoiceSecretError(
            "could not stage the HOLYROG XTTS secret file"
        ) from exc
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.install_secret:
            ...
            _write(first)
            print(
                "HOLYROG XTTS secret stored in Windows Credential Manager "
        ...
        if args.derive_from_compute_worker:
            ...
            print(
                "HOLYROG XTTS secret derived from the local compute-worker "
                "credential without printing either value."
            )
            return 0
        if args.remove_secret:
            ...
            try:
                ...
                if _credential_error_code(exc) in {2, 1168}:
                    ...
                    raise VoiceSecretError(
                        "could not remove the HOLYROG XTTS credential"
                    ) from exc
            return 0
        stored = _read()
        if stored is None:
            raise VoiceSecretError(
                "HOLYROG XTTS authorization is not configured; use --install-secret"
            )
